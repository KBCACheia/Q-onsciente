$(function(){
	...
	function correcao(){ /*compara os vetores de respostas do usuário e do JSON*/
		var acerto=0;
		for (i = 0; i < respostas.length; i++){
			if (respostas[i]==respostasPerguntas[i]){
				acerto++;
			}
		}
		return acerto;
	...
	$("#btProxima").click(function(){ /* botão Proxima chama o metodo para carregar a pergunta*/
		if (! $("input[name='radio-choice-v-2']").is(':checked')){
			...
			if (indexPergunta == numeroPerguntas){ /*mostra resultado*/
				...
				var num = correcao();
				$("#redResultado").html( "Você " +(num===0 ? "não ": "")+"acertou " +(num===0 ? "nenhuma ": num) + (num <= 1 ? " questão!" : " questões!") + (num < 1 ? "</br>:( </br> Tente de Novo!</br>:D" : ""));
				gabarito();
			} else {
				carregarPergunta();
